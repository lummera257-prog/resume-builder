...
export default function Projects() {
  ...
  return (
    <SectionCard title="Projects" icon="🚀" enabled={enabled}
      onToggle={() => toggleSection('projects')} badge={items.length > 0 ? `${items.length}` : undefined}>

      {items.map((proj, i) => (
        <EntryCard key={proj.id}
          ...
          <div className="grid grid-cols-2 gap-2 pr-16">
            <div className="col-span-2">
              <Field label="Project Name">
                <TextInput value={proj.name} onChange={v => updateItem('projects', proj.id, { name: v })}
                  placeholder="AI Resume Builder" />
              </Field>
            </div>
            <div className="col-span-2">
              <Field label="Description" hint="What it does, your role, and impact/results">
                <TextArea value={proj.description}
                  onChange={v => updateItem('projects', proj.id, { description: v })}
                  rows={3}
                  placeholder="Built a full-stack resume builder used by 5,000+ users. Reduced resume creation time by 70% with real-time preview and one-click PDF export." />
              </Field>
            </div>
            <div className="col-span-2">
              <Field label="Technologies Used">
                <TextInput value={proj.technologies}
            ...
            <div className="col-span-2">
              <Field label="Link (GitHub / Live URL)">
                <TextInput value={proj.link} onChange={v => updateItem('projects', proj.id, { link: v })}
