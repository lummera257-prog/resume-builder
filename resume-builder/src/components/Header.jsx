      ...
      <div className="flex items-center gap-3 px-4 h-14 max-w-screen-2xl mx-auto">
        ...
        <div className="flex items-center gap-2 mr-2">
          ...
          <div className="hidden sm:block">
            ...
            <span className="ml-1.5 text-xs text-slate-400 font-medium hidden md:inline">Free Resume Builder</span>
          </div>
        </div>
          ...
          {showConfirm ? (
            <div className="flex items-center gap-1">
              ...
              <button onClick={() => { resetResume(); setShowConfirm(false) }}
                className="text-xs px-2 py-1 bg-red-100 text-red-600 rounded hover:bg-red-200 transition-colors">Yes</button>
              <button onClick={() => setShowConfirm(false)}
                className="text-xs px-2 py-1 bg-slate-100 text-slate-600 rounded hover:bg-slate-200 transition-colors">No</button>
            </div>
          ) : (
