...
export function SectionCard({
  ...
  return (
    <div className={`form-section transition-opacity ${enabled === false ? 'opacity-60' : ''}`}>
      {/* Header */}
      <div
        className="flex items-center gap-2 px-4 py-3 bg-slate-50 border-b border-slate-100 cursor-pointer select-none"
        onClick={() => setOpen((o) => !o)}
      >
...
export function AddButton({ onClick, label = 'Add Entry' }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="w-full mt-1 py-2 border-2 border-dashed border-slate-200 rounded-lg text-sm text-slate-500 hover:border-blue-400 hover:text-blue-600 hover:bg-blue-50/50 transition-all duration-150 font-medium"
    >
      + {label}
    </button>
  )
