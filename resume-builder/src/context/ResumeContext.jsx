...
function resumeReducer(state, action) {
  switch (action.type) {
    ...
    case 'UPDATE_ITEM': {
      ...
      return {
        ...state,
        [section]: state[section].map(i => i.id === id ? { ...i, ...data } : i),
      }
    }

    case 'REMOVE_ITEM': {
      const { section, id } = action.payload
      return { ...state, [section]: state[section].filter(i => i.id !== id) }
    }
    ...
    case 'ADD_CUSTOM_SECTION':
      return {
        ...state,
        customSections: [...state.customSections, { id: genId(), title: 'Custom Section', entries: [] }]
      }
